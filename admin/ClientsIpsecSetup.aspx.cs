...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean InsertIpsecClient(String clientid, String value ,String assetclass)
    {
        ...
        try
        {
            conn.Close();
            conn.Open();
            String query = "INSERT INTO Ipsecs(client_id ,value,allocation_level ,asset_class_id) values('" + clientid + "','" + value + "','client', '"+ assetclass+"')";
            SqlCommand cmd = new SqlCommand(query, conn);
            cmd.ExecuteReader(CommandBehavior.CloseConnection);
            added = true;
        }
...

    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select  id,asset_class_id as [Asset Class Name] ,Value from Ipsecs where client_id='" +  txtID.Text +"' and Asset_Manager_id ='"+rdAssetMangers.SelectedItem.Text +"' and active='1' order by id desc", conn);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
...

    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select  id,asset_class_id as [Asset Class Name] ,Value ,allocation_level as Level from Ipsecs where client_id='" + txtID.Text + "' and allocation_level='client' and active='1' order by id desc", conn);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataSet ds = new DataSet("ds");
            da.Fill(ds);
    ...
    protected void Button3_Click1(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                MsgBox("Asset Class Captured Successfully", Page, this);
            }
            getIpsecClient();
