...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public void loadreport(String clientid,String quarter, String year)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select z.asset_class_id as [Asset Class], round(sum(z.value),2) as Total, round((sum(z.value)/(select sum(value) from investments where client_id='" + txtID.Text + "' and quarter='" + rdQuarter.SelectedItem.Value + "' and year='" + cmbYear.SelectedItem.Text + "')),2)* 100 as Exposure, Corridor, concat(i.value,'%') as  Target,g.IpsecRegulatory as Ipseclimit from [Intellego].[dbo].[Investments] z left join Ipsecs i on i.Asset_class_id= z.Asset_class_id and i.client_id= z.client_id   left join IpsecGlobalRegulatory g on i.Asset_class_id=g.AssetClass   where  z.active='1' and z.year='" + cmbYear.SelectedItem.Text + "' and z.quarter='" + rdQuarter.SelectedItem.Value + "' and i.allocation_level='client' and z.client_id='" + txtID.Text + "' group by z.Asset_class_id,i.Corridor,i.value ,g.IpsecRegulatory union all select top 1 'Total','0','0','','0','' from Investments", conn);
