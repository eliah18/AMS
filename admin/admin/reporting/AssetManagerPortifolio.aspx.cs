...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public void loadreport(String clientid,String quarter, String year)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("AssetManagerPortifolioProfiles", conn);
    ...
    public void getClassColors()
    {
        double sum = 0;
        foreach (GridViewRow row in grdPortifolioSnapshot.Rows)
        {
             
            if (row.Cells[0].Text == "Cash")
            {
                row.BackColor= System.Drawing.Color.Pink;
            }
            if (row.Cells[0].Text == "listed property")
            {
                row.BackColor = System.Drawing.Color.Pink;
            }
            if (row.Cells[0].Text == "Bonds and Debentures")
            {
                row.BackColor = System.Drawing.Color.Pink;
            }
            if (row.Cells[0].Text == "listed equities")
            {
                row.BackColor = System.Drawing.Color.Pink;
            }
            if (row.Cells[0].Text == "Guaranteed Fund")
            {
                row.BackColor = System.Drawing.Color.Pink;
            }
            if (row.Cells[0].Text == "Money Market")
            {
                row.BackColor = System.Drawing.Color.Yellow;
            }
            if (row.Cells[0].Text == "Unlisted Equity")
            {
                row.BackColor = System.Drawing.Color.Yellow;
            }
            if (row.Cells[0].Text == "Unlisted Property")
            {
                row.BackColor = System.Drawing.Color.Yellow;
            }
            if (row.Cells[0].Text == "Alternative Investments")
            {
                row.BackColor = System.Drawing.Color.Yellow;
            }
            if (row.Cells[0].Text == "Foreign Equity")
            {
                row.BackColor = System.Drawing.Color.Yellow;
            }
