...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean InsertIpsec (String clientid, String assetclassid, String assetmanagerid , String value , String Corridor)
    {
        Boolean added = false;
        try
        {
            ...
            String query = "INSERT INTO Ipsecs(client_id ,Asset_class_id, Asset_manager_id ,value,allocation_level , Corridor) values('" + clientid + "','" + assetclassid + "','" + assetmanagerid + "','" + value + "','assetmanager' ,'" + Corridor + "')";
    ...
    public Boolean InsertIpsecClient(String clientid, String value ,String assetclass,String Corridor)
    {
        Boolean added = false;
        try
        {
            ...
            String query = "INSERT INTO Ipsecs(client_id ,value,allocation_level ,asset_class_id ,asset_manager_id ,Corridor) values('" + clientid + "','" + value + "','client', '"+ assetclass+"' ,'"+ txtFirstName.Text+ "','" + Corridor + "')";
            SqlCommand cmd = new SqlCommand(query, conn);
            cmd.ExecuteReader(CommandBehavior.CloseConnection);
            added = true;
        }
...

    {
        ...
        try
        {
            ...
            {
                cmd = new SqlCommand("select  id,asset_class_id as [Asset Class Name] ,Value,Corridor as Range  from Ipsecs where client_id='" + txtID.Text + "'  and active='1' and allocation_level='client' order by id desc", conn);
            }
            else if(RadioButtonList1.SelectedItem.Text== "AssetManager")
            {
    ...
    public Boolean updatedata(String id,String value, String corridor)
    {
        bool added = false;
        conn.Close();
        conn.Open();
        SqlCommand cmd = new SqlCommand("update  Ipsecs set value='" + value + "',Corridor='"+corridor+"' where Id='" + id + "' ", conn);
        if ((conn.State == ConnectionState.Open))
            conn.Close();
        conn.Open();
    ...
    protected void Button3_Click1(object sender, EventArgs e)
    {
        ...
        if (RadioButtonList1.SelectedItem.Value == "Client") {
            ...
            {
                if (InsertIpsecClient(txtID.Text, txtAllocation.Text, cmbAsetClass.SelectedItem.Text,txtCorridor.Text))
                {
        ...
        else
        {
            ...
            {
                if (InsertIpsec(txtID.Text, cmbAsetClass.SelectedItem.Text, rdAssetMangers.SelectedItem.Text, txtAllocation.Text ,txtCorridor.Text))
                {
                    MsgBox("Asset Allocation Successfully Captured", Page, this);
                    getIpsecClient();
                    clearfields();
    ...
    {
        if (updatedata(txtClientIpsecUpdateID.Text, txtAllocation.Text,txtCorridor.Text))
        {
            MsgBox("Update Successful", this.Page, this);
            //getIpsec();
            getIpsecClient();
