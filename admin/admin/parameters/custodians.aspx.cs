...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean checkcustodian(  String surname)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM custodians where name='" + surname + "'   ", conn);
    ...
    private void GetListData()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select id,name,contact_details,address from custodians order by id desc ", conn);
    ...
    public void fetcheditadata(String id)
    {
        try
        {
            conn.Close();
            conn.Open();
            string Query = "SELECT * FROM custodians WHERE id='" + id.ToString() + "' ";
            SqlCommand cmd = new SqlCommand(Query, conn);
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (user)
        {
            MsgBox("Custodian Already Exists", this.Page, this);
        ...
        else
        {
            ...
            if (add)
            {
                MsgBox("Custodian  successfully added", this.Page, this);
                GetListData();
    ...
    public Boolean editcustodian(string  id)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("update custodians set name='" + txtSurname.Text + "',contact_details='" + txtContactDetails.Text + "',address = '" + txtAddress.Text + "' where id= '" + id + "'", conn);
            if ((conn.State == ConnectionState.Open))
                conn.Close();
            conn.Open();
            cmd.ExecuteNonQuery();
    ...
    {
        Response.Redirect("custodians.aspx");
    }
}