...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean checkrole( String firstname)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM roles where role_name='" + firstname + "'   ", conn);
    ...
    public Boolean addrole(String firstname)
    {
        Boolean added = false;
        try
        {
            conn.Close();
            conn.Open();
            String query = "INSERT INTO roles(role_name) values('" + firstname + "')";
    ...
    private void GetListData()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select id,role_name as name from roles order by id desc ", conn);
    ...
    public void linkDiscard(object sender, System.EventArgs e)
    {
      ...
      SqlCommand  cmd = new SqlCommand("Delete from roles where Id='" + idd + "' ", conn);
    ...
    public void fetcheditadata(String id)
    {
        try
        {
            ...
            if (dr.Read() == true)
            {
                txtFirstName.Text = dr["role_name"].ToString();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        Boolean user = checkrole(txtFirstName.Text);
        if (user)
        {
            MsgBox("Role Already Exists", this.Page, this);

        }
        else
        {
            Boolean add = addrole(txtFirstName.Text);
            if (add)
            {
                MsgBox("Role successfully added", this.Page, this);
                GetListData();
    ...
    public Boolean edituser(string  id)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("update roles set role_name='" + txtFirstName.Text + "'   where id= '" + id + "'", conn);
            if ((conn.State == ConnectionState.Open))
                conn.Close();
            conn.Open();
            cmd.ExecuteNonQuery();
