...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean checkcustodian(  String surname)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM actuary where name='" + surname + "'   ", conn);
    ...
    public Boolean addcustodian(String surname,String contactdetails,String address)
    {
        ...
        try
        {
            conn.Close();
            conn.Open();
            String query = "INSERT INTO actuary(name,contact_details,address) values('" + surname + "','" + contactdetails + "','" + address + "')";
    ...
    private void GetListData()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select id,name,contact_details,address from actuary order by id desc ", conn);
    ...
    public void linkDiscard(object sender, System.EventArgs e)
    {
      ...
      SqlCommand  cmd = new SqlCommand("Delete from actuary where Id='" + idd + "' ", conn);
    ...
    public void fetcheditadata(String id)
    {
        try
        {
            conn.Close();
            conn.Open();
            string Query = "SELECT * FROM actuary WHERE id='" + id.ToString() + "' ";
            SqlCommand cmd = new SqlCommand(Query, conn);
            ...
            if (dr.Read() == true)
            {
                ...
                txtSurname.Text= dr["name"].ToString();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (user)
        {
            MsgBox("Actuary Already Exists", this.Page, this);
            

        }
        else
        {
            Boolean add = addcustodian( txtSurname.Text,txtContactDetails.Text,txtAddress.Text);
            if (add)
            {
                MsgBox("Actuary successfully added", this.Page, this);
                GetListData();
    ...
    public Boolean editcustodian(string  id)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("update actuary set name='" + txtSurname.Text + "',contact_details='" + txtContactDetails.Text + "',address = '" + txtAddress.Text + "' where id= '" + id + "'", conn);
            if ((conn.State == ConnectionState.Open))
                conn.Close();
            conn.Open();
            cmd.ExecuteNonQuery();
    ...
    {
        Response.Redirect("Actuary.aspx");
    }
