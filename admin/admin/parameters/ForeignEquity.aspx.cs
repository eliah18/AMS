...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean addprice(String type,String name)
    {
        ...
        try
        {
            conn.Close();
            conn.Open();
            String query = "INSERT INTO foreign_equities(sector , company) values('" + type + "','" + name + "')";
    ...
    private void GetListData()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select id,sector as [Equity Type],company as [Company Name]  from foreign_equities where active='1' order by id desc  ", conn);
    ...
    public void linkDiscard(object sender, System.EventArgs e)
    {
      ...
      SqlCommand  cmd = new SqlCommand("update  foreign_equities set active='0' where Id='" + idd + "' ", conn);
    ...
    public void fetcheditadata(String id)
    {
        try
        {
            conn.Close();
            conn.Open();
            string Query = "SELECT * FROM  foreign_equities WHERE id='" + id.ToString() + "' and active='1' ";
            SqlCommand cmd = new SqlCommand(Query, conn);
            ...
            if (dr.Read() == true)
            {
                ...
                cmbCounter.SelectedValue= dr["sector"].ToString();
                txtBond.Text = dr["company"].ToString();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (user)
        {
            MsgBox("Foreign Equity Already Exists", this.Page, this);
    ...
    public Boolean editcustodian(string  id)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("update foreign_equities set sector ='" + cmbCounter.SelectedItem.Value + "', company='" + txtBond.Text + "' where id= '" + id + "'", conn);
            if ((conn.State == ConnectionState.Open))
                conn.Close();
            conn.Open();
            cmd.ExecuteNonQuery();
    ...
    {
        Response.Redirect("ForeignEquity.aspx");
    }
