...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    protected void prePopulateQuarterInfo()
    {
        rdQuarter.SelectedValue = Request.QueryString["quarter"];
        cmbYear.SelectedValue = Request.QueryString["year"];
        if (rdQuarter.SelectedValue == "4")
        {
            rdQuarterTo.SelectedValue = "1";
            cmbYearTo.SelectedValue = (int.Parse(cmbYear.SelectedValue.ToString()) + 1).ToString();

        }
        else
        {
            rdQuarterTo.SelectedValue = (int.Parse(rdQuarter.SelectedValue) + 1).ToString();
            cmbYearTo.SelectedValue = cmbYear.SelectedValue;
        }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //Session["SelectedQuart"] = rdQuarter.SelectedValue;
            //Session["SelectedYear"] = cmbYear.SelectedValue;
...
public void loadYear()
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (cmbYearTo.SelectedItem.Text == "Select Year")
        {
            MsgBox("Please Select To (Year)", this.Page, this);
            return;
        }
        if (rdQuarterTo.SelectedValue == "")
        {
            MsgBox("Please Select To (Quarter)", this.Page, this);
            return;
        }
        //MsgBox("Please Select Quarter", this.Page, this);
...

    {
        if (cmbYear.SelectedItem.Text == "Select Year")
        {
            MsgBox("Please Select From (Year)", this.Page, this);
            return;
        }
        if (rdQuarter.SelectedValue == "")
        {
        ...
        {
            //MsgBox("client_id=" + txtID.Text + "&prevQuarter=" + rdQuarter.SelectedValue + "&prevYear=" + cmbYear.SelectedItem.Text + " &toQuarter =" + rdQuarterTo.SelectedValue + " &toYear =" + cmbYearTo.SelectedItem.Text, this.Page, this);

            string prevQuarter = rdQuarter.SelectedValue;
            string prevYear = cmbYear.SelectedItem.Text;
            string toQuarter = rdQuarterTo.SelectedValue;
            string toYear = cmbYearTo.SelectedItem.Text;

            string strscript;
            strscript = "<script langauage=JavaScript>";
            strscript += "window.open('assetQ2Q.aspx?client_id=" + txtID.Text + "&prevQuarter=" + prevQuarter + "&prevYear=" + prevYear + "&toQuarter="+ toQuarter + "&toYear=" + toYear + "');";
            strscript += "</script>";
