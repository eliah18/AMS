...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    private void GetListData()
    {
        ...
        try
        {
            ...
            conn.Open();
            String query = "SELECT  a.id,a.id as investmentid,a. asset_name,'0' AS [Amount],case when a.linkstatus='1' then 'needButton' else 'needButton' end as ControlType FROM assets_class a where a.active=1 ";
            SqlCommand cmd = new SqlCommand(query, conn);
      ...
      //  try
        {
            ...
            //LastColumn = grdApps.Rows.Count;
            //MsgBox("LastColumn index: " + LastColumn.ToString(), this.Page, this);
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        else if (rdAssetMangers.SelectedItem.Text == "Select An Asset Manager")
        {
        ...
        foreach (GridViewRow row in grdApps.Rows)
        {
            ...
            sum = 0;
                for (int b=1;b < row.Cells.Count-1; b++)
                {
                    ...
                    int colCount1 = grdApps.Rows[0].Cells.Count - 1;

                    row.Cells[colCount1].Text = sum.ToString();
                    row.Cells[colCount1].Font.Bold = true;
                    row.Cells[b].Text = Decimal.Parse(row.Cells[b].Text).ToString("n");
    ...
    protected void linkDiscard_Click(object sender, EventArgs e) // qwerty
    {
    ...
    protected void Button1_ClickYes(object sender, EventArgs e) 
    {
        String assetmanager = rdAssetMangers.SelectedItem.Value.ToString();
        String clientid = Request.QueryString["clientid"];
        ...
        //if (clearAssetTable(clientid, year, quarter, user))
        //{
        //conn.Close();
        //conn.Open();
        //SqlCommand cmd = new SqlCommand("update  Ipsecs set value='" + value + "' where Id='" + id + "' ", conn);
        ...
        //conn.Close();
        //this.subaccount = Request.QueryString["subaccount"];
        //MsgBox("subaccount" + rdSubAccount.SelectedItem.Value.ToString(), this.Page, this);
            ...
            try
            {
                ...
                String query = "update Investments set active=0,DeactivatedBy='" + user + "',DeactivatedDate=getdate() where client_id='" + clientid + "' and year='" + year + "' and quarter='" + quarter + "' and subaccount='" + rdSubAccount.SelectedItem.Value.ToString() + "' and Asset_manager_id='"+ assetmanager + "'";            
            ...
            //cmd.ExecuteReader(CommandBehavior.CloseConnection);         tfvygbhnjmk
            //gyhunjmk,l
            int a = cmd.ExecuteNonQuery();
            conn.Close();
            //if (a == 1)
            //{
                MsgBox("Table Cleared Successfully", this.Page, this);
                Removelinked_assetclasses();
            //}           
    ...
    protected void Removelinked_assetclasses()
    {
        MsgBox("Enter Removelinked_assetclasses", this.Page, this);
        String assetmanager = rdAssetMangers.SelectedItem.Value.ToString();
        String clientid = Request.QueryString["clientid"];
        String assetclass = Request.QueryString["data"];
        String quarter = Request.QueryString["quarter"];
        ...
        {            
            ...
            String query1 = "update linked_assetclasses set active=0 where client_id='" + clientid + "' and year='" + year + "' and quarter='" + quarter + "' and subaccount='" + rdSubAccount.SelectedItem.Value.ToString() + "' and Assetmanager='" + assetmanager + "' ";
           ...
           // MsgBox("Table Cleared Successfully2", this.Page, this);
            //lblwarning.Visible = false;
            //btnNo.Visible = false;
            //btnYes.Visible = false;
        }
    ...
    protected void Button1_ClickNo(object sender, EventArgs e)
    {
        lblwarning.Visible = false;
        btnNo.Visible = false;
        btnYes.Visible = false;
    ...
    //public Boolean clearAssetTable(String clientid, String year, String quarter,String user)
    //{
    //    Boolean added = false;
    //    this.subaccount = Request.QueryString["subaccount"];
    //    MsgBox("this.subaccount" + this.subaccount, this.Page, this);
    //    try
    //    {
    //        conn.Close();
    //        conn.Open();
            
    //        String query = "update Investments set active=0,DeactivatedBy='"+user+"',DeactivatedDate=getdate() where client_id='"+clientid+"' and year='"+year+"' and quarter='"+quarter+"' and subaccount='" + this.subaccount + "'";
    //        SqlCommand cmd = new SqlCommand(query, conn);
    //        cmd.ExecuteReader(CommandBehavior.CloseConnection);
    //        added = true;
    //    }
    ...
    //    strscript += "window.open('assetexposure.aspx?client_id=" + txtID.Text + "&Quarter=" + txtQuarter.Text + "&Subaccount=" + rdSubAccount.SelectedItem.Text + "&Year=" + txtYear.Text + "');";
    //    strscript += "</script>";
    ...
    public void loadSubAccount()
    {
        ...
        string com = " select  * from  client_subaccounts_relations where client_id='" + txtID.Text + "' and active=1";
        SqlDataAdapter adpt = new SqlDataAdapter(com, conn);
        DataTable dt = new DataTable();
        adpt.Fill(dt);
