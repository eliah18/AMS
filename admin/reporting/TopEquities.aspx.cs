...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public void loadreport(String clientid,String quarter, String year)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("WITH T2 AS (SELECT Counter as BondCounter,concat(round((cast(cast(exposure as DECIMAL(9,6)) as float) ),2),'%') as BondExposure,ROW_NUMBER()OVER(ORDER BY (SELECT 1))AS ID FROM [getTop5Bonds]('2','" + @clientid + "','" + @year + "','" + @quarter + "')),T1 AS((SELECT Counter as EquityCounter, concat(round((cast(cast(exposure as DECIMAL(9, 6)) as float)), 2), '%') as EquityExposure, ROW_NUMBER()OVER(ORDER BY(SELECT 1))AS ID FROM[getTop5Equties]('3', '" + @clientid + "', '" + @year + "', '" + quarter + "'))) SELECT T1.Equitycounter,T1.EquityExposure,T2.BondCounter,T2.BondExposure FROM T1 FULL JOIN T2 ON(T1.ID = T2.ID)", conn);
            //SqlCommand cmd = new SqlCommand("WITH T3 AS((SELECT Counter as ForeignEquityCounter, concat(round((cast(cast(exposure as DECIMAL(9, 6)) as float)), 2), '%') as ForeignEquityExposure, ROW_NUMBER()OVER(ORDER BY(SELECT 1))AS ID FROM[getTop5ForeignEquties]('5', '" + @clientid + "', '" + @year + "', '" + @quarter + "'))),T2 AS (SELECT Counter as BondCounter,concat(round((cast(cast(exposure as DECIMAL(9,6)) as float) ),2),'%') as BondExposure,ROW_NUMBER()OVER(ORDER BY (SELECT 1))AS ID FROM [getTop5Bonds]('2','" + @clientid + "','" + @year + "','" + @quarter + "')),T1 AS((SELECT Counter as EquityCounter, concat(round((cast(cast(exposure as DECIMAL(9, 6)) as float)), 2), '%') as EquityExposure, ROW_NUMBER()OVER(ORDER BY(SELECT 1))AS ID FROM[getTop5Equties]('3', '" + @clientid + "', '" + @year + "', '" + @quarter + "')))SELECT T1.Equitycounter,T1.EquityExposure,T2.BondCounter,T2.BondExposure,T3.ForeignEquityCounter,T3.ForeignEquityExposure FROM T1 FULL JOIN T2 ON(T1.ID = T2.ID) JOIN T3 ON (T2.ID=T3.ID)", conn);
