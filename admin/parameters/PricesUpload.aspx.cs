...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean checkPrices(String pricedate)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM prices where CAST(PriceDate AS DATE)=convert(date,'" + pricedate + "') ", conn);
    ...
    private void GetListData()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select id,PriceDate,Company,convert(Double Precision,ClosingPrice) as [VWAP] from prices where convert(date,PriceDate)= convert(date,'" + txtDate.Text + "') and ClosingPrice<>' ' order by company", conn);
    ...
    public void UploadData()
    {
...

        {
            ...
            {
                MsgBox("Upload Successful", this.Page, this);
                GetListData();
                grdpanel.Visible = false;
            ...
            {
                MsgBox("Upload Failed", this.Page, this);
