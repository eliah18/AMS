...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean checkprice(  String surname)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM unlisted_property where propertyname='" + surname + "'   ", conn);
    ...
    public Boolean addprice(String type,String name)
    {
        ...
        try
        {
            conn.Close();
            conn.Open();
            String query = "INSERT INTO unlisted_property(propertytype , propertyname) values('" + type + "','" + name + "')";
    ...
    private void GetListData()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select id,propertytype as [Property Type],propertyname as [Property Name]  from unlisted_property where active='1' order by id desc  ", conn);
    ...
    public void linkDiscard(object sender, System.EventArgs e)
    {
      ...
      SqlCommand  cmd = new SqlCommand("update  unlisted_property set active='0' where Id='" + idd + "' ", conn);
    ...
    public void fetcheditadata(String id)
    {
        try
        {
            conn.Close();
            conn.Open();
            string Query = "SELECT * FROM unlisted_property WHERE id='" + id.ToString() + "' and active='1' ";
            SqlCommand cmd = new SqlCommand(Query, conn);
            ...
            if (dr.Read() == true)
            {
                ...
                cmbCounter.SelectedValue= dr["propertytype"].ToString();
                txtBond.Text = dr["propertyname"].ToString();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (user)
        {
            MsgBox("Property  Already Exists", this.Page, this);
        ...
        else
        {
            ...
            if (txtBond.Text == "0")
            {
                MsgBox("Enter Property Name", this.Page, this);
                return;
            }
            ...
            if (add)
            {
                MsgBox("Property  successfully added", this.Page, this);
                GetListData();
    ...
    public Boolean editcustodian(string  id)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("update unlisted_property set propertytype ='" + cmbCounter.SelectedItem.Value + "', propertyname='" + txtBond.Text + "' where id= '" + id + "'", conn);
            if ((conn.State == ConnectionState.Open))
                conn.Close();
            conn.Open();
            cmd.ExecuteNonQuery();
    ...
    {
        Response.Redirect("UnlistedProperty.aspx");
    }
    ...
    public void recordinsert(List<string> li)
    {
        //try
        {
            ...
            foreach (string item in li)
            {
                ...
                stringbi.AppendFormat("{0}('UnlistedProperty','" + txtBond.Text + "','{1}');", query, item);
            }
            conn.Close();
            conn.Open();
    ...
    private void GetSelectedAssetManagers()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand(" select b.id,a.surname  as AssetManager  from asset_managers a join assetmanager_unlistedclassholdings b on a.id=b.assetmanager_id  where b.propertyclass='UnlistedProperty' and b.active=1 and b.propertyname='" + txtBond.Text + "'   ", conn);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataSet ds = new DataSet("ds");
            da.Fill(ds);
