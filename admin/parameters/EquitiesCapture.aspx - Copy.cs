...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean addprice(String date)
    {
        ...
        //try
        {
            ...
            SqlCommand cmd = new SqlCommand("  DECLARE @Date date = '" + date + "' SELECT B.Company_name,B.price_best,B.price_date FROM[testcds_ROUTER].[dbo].[prices] B JOIN(SELECT MAX(A.PriceID) AS LastID, A.Company_Name FROM[testcds_ROUTER].[dbo].[prices] A WHERE CONVERT(DATE, A.price_date)=@Date GROUP BY A.Company_Name) C ON B.PriceID=C.LastID ORDER BY B.Company_name", conprice);
