...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean Adduser(String firstname,String date,String ratereturn,String specialnotes,String fees,String custodian,String contact,String address ,String administrators, String clientnumber)
    {
        ...
        try
        {
            conn.Close();
            conn.Open();
            String query = "INSERT INTO clients(name,date_of_take,rate_of_return,special_notes,fees,custodian,contact_details,address,administrators,client_number,benchmark ,sponsor,type,email,feevalue) values('" + firstname + "','" + date + "','" + ratereturn + "','" + specialnotes + "','" + fees + "','" + custodian + "','" + contact + "','" + address + "','" + administrators + "' ,'" + clientnumber + "' ,'" + txtBenchmark.Text + "','" + txtSponsor.Text + "' ,'" + rdClientType.Text + "','" + txtemail.Text + "','" + txtValue.Text + "')";
    ...
    private void GetSelectedAssetManagers()
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand(" select c.id as ID ,a.surname as Name from client_assetmanager_relations c join asset_managers a  on c.assetmanager_id=a.id where  c.client_id='" + txtgenerator.Text +"' and c.active='1' ", conn);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataSet ds = new DataSet("ds");
            da.Fill(ds);
    ...
    private void GetSelectedSubAcc()
    {
        conn.Close();
        try
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand(" select c.id,s.subaccountname from client_subaccounts_relations c join subaccounts s on c.subaccount=s.id where c.client_id='" + txtgenerator.Text + "' and c.active='1' ", conn);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataSet ds = new DataSet("ds");
            da.Fill(ds);
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    grdSubAcc.DataSource = ds;
                    grdSubAcc.DataBind();
                }
            }
            else
            {
                grdSubAcc.DataSource = null;
                grdSubAcc.DataBind();
            }
        }
        catch (Exception ex)
    ...
    public void viewprofile(object sender, System.EventArgs e)
    {
        string idd = ((LinkButton)sender).CommandArgument;
        string strPopup = "<script language='javascript' ID='script1'>"

        // Passing intId to popup window.
        + "window.open('ViewAssetmanagersProfile.aspx?name=" + HttpUtility.UrlEncode(idd.ToString()) 
    ...
    {
        string idd = ((LinkButton)sender).CommandArgument;
        SqlCommand cmd = new SqlCommand("update client_assetmanager_relations   set active='0' where Id='" + idd + "' ", conn);
        if ((conn.State == ConnectionState.Open))
            conn.Close();
        conn.Open();
        cmd.ExecuteNonQuery();
        conn.Close();
        MsgBox("Asset Manager Successfully Removed", this.Page, this);
        // GetListData();
        GetSelectedAssetManagers();


    }
    public void linkDiscardSubAcc(object sender, System.EventArgs e)
    {
        string idd = ((LinkButton)sender).CommandArgument;
        SqlCommand cmd = new SqlCommand("update client_subaccounts_relations set active='0' where Id='" + idd + "' ", conn);
        if ((conn.State == ConnectionState.Open))
            conn.Close();
        conn.Open();
        cmd.ExecuteNonQuery();
        conn.Close();
        MsgBox("Sub Account Successfully Removed", this.Page, this);
        // GetListData();
        GetSelectedSubAcc();
    ...
    public void fetcheditadata(String id)
    {
        try
        {
            ...
            if (dr.Read() == true)
            {
                ...
                rdFees.Text= dr["fees"].ToString();

                rdClientType.Text = dr["type"].ToString();
               // cmbActuary.Text = dr["actuary"].ToString();
                cmbCustodians.Text = dr["custodian"].ToString();
                cmbAdministrators.Text = dr["administrators"].ToString();
                
                txtContactDetails.Text= dr["contact_details"].ToString();
                //txtIPS.Text = dr["IPS"].ToString();
                txtAddress.Text = dr["address"].ToString();
                txtgenerator.Text= dr["client_number"].ToString();
                txtSponsor.Text= dr["sponsor"].ToString();
                txtemail.Text = dr["email"].ToString();
                txtReturn.Text= dr["rate_of_return"].ToString();
                if (dr["fees"].ToString() == "Fixed Amount")
                {
                    Label3.Visible = true;
                    txtValue.Text = dr["feevalue"].ToString();
                    txtValue.Visible = true;
                    txtPercentValue.Visible = false;
                }
                else
                {
                    Label3.Visible = true;
                    txtValue.Visible = false;
                    txtPercentValue.Visible = true;
                    txtPercentValue.Text = dr["feevalue"].ToString();
                }
                
                LbselecdtedAssetmanagers.Visible = true;
                GetSelectedAssetManagers();
                lblSelectedSubAcc.Visible = true;
                GetSelectedSubAcc();
                Panel1.Visible = true;
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            if (rdFees.SelectedItem.Text == "Fixed Amount")
            {
                String value = txtValue.Text;
            }
            else
            {
                String value = txtPercentValue.Text;
            }
           
            Boolean add = Adduser(txtFirstName.Text, txtDate.Text,txtReturn.Text,txtSpecialnotes.Text, rdFees.SelectedItem.Text,cmbCustodians.SelectedItem.Text,txtContactDetails.Text,txtAddress.Text,cmbAdministrators.SelectedItem.Text, txtgenerator.Text);
            if (add)
            {
    ...
    public Boolean edituser(string  id)
    {
        ...
        {
            value = txtPercentValue.Text;
        }
        {
            SqlCommand cmd = new SqlCommand("update clients set name='" + txtFirstName.Text + "' ,date_of_take='" + txtDate.Text + "',rate_of_return='" + txtReturn.Text + "',special_notes='" + txtSpecialnotes.Text + "' ,fees='" + rdFees.SelectedItem.Text + "' ,custodian= '" + cmbCustodians.SelectedItem.Text + "',contact_details='" + txtContactDetails.Text + "',address='" + txtAddress.Text + "',administrators='" + cmbAdministrators.SelectedItem.Text + "' ,benchmark='"+ txtBenchmark.Text + "' , sponsor='" + txtSponsor.Text + "',email='" + txtemail.Text + "' ,type='" + rdClientType.Text + "', feevalue='"+ value +"'  where id= '" + id + "'", conn);
            if ((conn.State == ConnectionState.Open))
                conn.Close();
            conn.Open();
            cmd.ExecuteNonQuery();
    ...
    public void loadSubAccounts()
    {
        ...
        DataTable dt = new DataTable();
        adpt.Fill(dt);
        cmbSubAcc.DataSource = dt;
        cmbSubAcc.DataBind();
        cmbSubAcc.DataTextField = "subaccountname";

        cmbSubAcc.DataValueField = "ID";
        cmbSubAcc.DataBind();
    ...
    {
        string com = "select * from Fees";
        SqlDataAdapter adpt = new SqlDataAdapter(com, conn);
        DataTable dt = new DataTable();
        adpt.Fill(dt);
        rdFees.DataSource = dt;
        rdFees.DataBind();
        rdFees.DataTextField = "fees";

        rdFees.DataValueField = "fees";
        rdFees.DataBind();
    ...
    public void loadcustodians()
    {
        ...
        cmbCustodians.DataValueField = "name";
        cmbCustodians.DataBind();
    ...
    public void loadAdministrators()
    {
      ...
      cmbAdministrators.DataTextField = "name";
        ...
        cmbAdministrators.DataValueField = "name";
        cmbAdministrators.DataBind();
    ...
    {
        string com = "select * from actuary";
        SqlDataAdapter adpt = new SqlDataAdapter(com, conn);
        DataTable dt = new DataTable();
        adpt.Fill(dt);
        cmbActuary.DataSource = dt;
        cmbActuary.DataBind();
        cmbActuary.DataTextField = "name";

        cmbActuary.DataValueField = "name";
        cmbActuary.DataBind();
    ...
    public void generateclientid()
    {
        try
        {
            conn.Close();
            conn.Open();
            string Query = "select max(id)+ 1 as generator from  Clients";
            SqlCommand cmd = new SqlCommand(Query, conn);
            SqlDataReader dr = cmd.ExecuteReader();
    ...
    protected void addSubAcc_Click(object sender, EventArgs e)
    {
        if (txtgenerator.Text == "")
        {
            txtgenerator.Text = "1";
        ...
        List<string> li = new List<string>();
        foreach (ListItem item in cmbSubAcc.Items)
        {
            if (item.Selected)
            {
                li.Add(item.Value);
    ...
    public void recordinsertSubacc(List<string> li)
    {
        //try
        {
            ...
            foreach (string item in li)
            {
                ...
                const string query = "insert into client_subaccounts_relations (client_id,subaccount,active) values";
                stringbi.AppendFormat("{0}('" + txtgenerator.Text + "','{1}','1');", query, item);
            }
            conn.Close();
            conn.Open();
            SqlCommand cmd = new SqlCommand();
            ...
            if (a >= 1)
            {
                MsgBox("Sub Account Added", this.Page, this);
            }
        }
    ...
    protected void rdFees_SelectedIndexChanged(object sender, EventArgs e)
    {
        if(rdFees.SelectedItem.Text== "Fixed Amount")
        {
            Label3.Visible = true;
