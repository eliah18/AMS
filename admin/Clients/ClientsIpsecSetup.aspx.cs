...
public partial class admin_parameters_User : System.Web.UI.Page
{
...

    {
        ...
        //{
            ...
            if (RadioButtonList1.SelectedItem.Text == "Client")
            {
                cmd = new SqlCommand("(select id,asset_class_id as [Asset Class Name] ,concat(value,' %') as [Value],Corridor as Range from Ipsecs where client_id='" + txtID.Text + "' and active='1' and allocation_level='client') union (select '0' + cast(id as varchar) as id, asset_name as [Asset Class Name], '0' as Value, '0' as Range from assets_class where asset_name not in (select asset_class_id from Ipsecs where client_id = '" + txtID.Text + "' and active = '1' and allocation_level = 'client')) order by id desc", conn);
            }
...

    {
        ...
        //{
            ...
            {
                cmd = new SqlCommand("select cast(id as varchar) as id,asset_class_id as [Asset Class Name] ,concat(Value,' %') as [Value],Corridor as Range from Ipsecs where client_id='" + txtID.Text + "' and  Asset_manager_id='" + rdAssetMangers.SelectedItem.Text + "' and active='1' union (select cast(id as varchar) as id, asset_name as [Asset Class Name], '0 %' as Value, '0' as Range from assets_class where asset_name not in (select asset_class_id from Ipsecs where client_id = '" + txtID.Text + "' and active = '1' and allocation_level = 'assetmanager')) order by id desc", conn);
            }
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataSet ds = new DataSet("ds");
            da.Fill(ds);
