...
public partial class admin_parameters_User : System.Web.UI.Page
{
    ...
    public Boolean checkClient(String name)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) from clients where name='" + name + "'", conn);
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        //else
        //{
            //if (checkClient(txtDate.Text))
            //{
            //    btnOverridePricesYes.Visible = true;
            //    btnOverridePricesNo.Visible = true;
            //    lblOverridePrices.Visible = true;
            //    Button1.Visible = false;
    ...
    protected void OverridePricesYes_Click(object sender, EventArgs e)
    {
        if (FileUpload1.HasFile)
        {
            DeleteExistingPrices();
            UploadData();
            btnOverridePricesYes.Visible = false;
            btnOverridePricesNo.Visible = false;
            lblOverridePrices.Visible = false;
            Button1.Visible = true;
        }
        else
        {
            MsgBox("Please Browse and Select File", this.Page, this);
            return;
        }

    }

    protected void OverridePricesNo_Click(object sender, EventArgs e)
    {
        btnOverridePricesYes.Visible = false;
        btnOverridePricesNo.Visible = false;
        lblOverridePrices.Visible = false;
    ...
    public void UploadData()
    {
...

        {
            ...
            {
                MsgBox("Invalid File Type", this.Page, this);
                return;
            }
            ...
            for (int i = 1; i < dtExcel.Rows.Count; i++)
            {
                try
                {
                    if (!(checkClient(dtExcel.Rows[i][0].ToString())))
                    {
                        conn.Close();
                        conn.Open();
                        //SqlCommand cmd = new SqlCommand("insert into prices(Company,ClosingPrice,DateCreated,PriceDate) values('" + dtExcel.Rows[i][1] + "', '" + dtExcel.Rows[i][2] + "',getdate(), '" + txtDate.Text + "')  ", conn);
                        generateclientid();
                        SqlCommand cmd = new SqlCommand("insert into clients (date_of_take,rate_of_return,special_notes,name,fees,benchmark,custodian,contact_details,address,administrators,client_number,email,type,sponsor,feevalue) values ('" + dtExcel.Rows[i][1] + "','" + dtExcel.Rows[i][2] + "','" + dtExcel.Rows[i][3] + "','" + dtExcel.Rows[i][0] + "','" + dtExcel.Rows[i][4] + "','" + dtExcel.Rows[i][9] + "','" + dtExcel.Rows[i][5] + "','" + dtExcel.Rows[i][6] + "','" + dtExcel.Rows[i][7] + "','" + dtExcel.Rows[i][8] + "','" + txtgenerator.Text.ToString() + "','" + dtExcel.Rows[i][12] + "','" + dtExcel.Rows[i][11] + "','" + dtExcel.Rows[i][10] + "','" + dtExcel.Rows[i][13] + "')", conn);
            ...
            {
                MsgBox("Client Upload Successful", this.Page, this);
                GetListData();
                grdpanel.Visible = false;
                usersPanel.Visible = true;
            }
            else if (count > 1 && count < dtExcel.Rows.Count)
            {
                MsgBox("New Clients Uploaded Successfully.", this.Page, this);
                GetListData();
                grdpanel.Visible = false;
                usersPanel.Visible = true;
            }
            ...
            {
                MsgBox("Client(s) In File Already Exist", this.Page, this);
            }
            else
            {
                MsgBox("Client Upload Failed", this.Page, this);
            }
        }
        else
        {
    ...
    public void generateclientid()
    {
        try
        {
            ...
            if (dr1.Read() == true)
            {                
                txtgenerator.Text = dr1["generator"].ToString();
            }
            dr1.Close();
        }
        catch (Exception ex)
